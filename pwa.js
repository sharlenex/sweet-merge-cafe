(() => {
  ...
  const artCache = 'sweet-merge-art-v2';
  const warmKey = 'sweet-merge-art-warmed-v2';
  const warmImages = [];
  const itemAssets = [
  ...
  const artAssets = [
    ...itemAssets,
    ...['coffee', 'cake', 'fruit'].flatMap(type => Array.from({ length: 12 }, (_, level) => `assets/machines/${type}-${level}.png`)),
